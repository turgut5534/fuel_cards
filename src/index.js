...
app.get('/cards/:id/transactions', async (req, res) => {
  ...
  try {
    ...
    const [latestSpendTransaction] = await db.query(
      'SELECT fuel_price FROM transactions WHERE card_id = ? AND transaction_type = "spend" ORDER BY transaction_date DESC LIMIT 1',
      [cardId]
    );
